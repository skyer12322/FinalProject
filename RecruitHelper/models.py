"""
Модуль с моделями данных для приложения RecruitHelper.
"""
...
class CUser(models.Model):
    ...
    resume = models.FileField(upload_to='resumes/',
                              null=True,
                              blank=True)
    user_vacancies = models.ManyToManyField("Vacancy",
                                            through='Application',
...
class Application(models.Model):
    ...
    candidate = models.ForeignKey("CUser",
                                  on_delete=models.CASCADE,
                                  related_name='applications')
    status = models.CharField(max_length=20,
                              choices=[("pending", "Pending"),
                                       ...
                                       ("rejected", "Rejected")],
                              default="pending")
    ai_rating = models.JSONField(default=list,
                                 blank=True)
...
class Chat(models.Model):
    ...
    user = models.ForeignKey('CUser', related_name='chats',
                             on_delete=models.SET_NULL,
                             null=True)
    company = models.ForeignKey('Company', related_name='chats',
                                on_delete=models.SET_NULL,
                                null=True)
...
class Vacancy(models.Model):
    ...
    geography = models.JSONField(default=None,
                                 blank=True,
                                 null=True)
    ai_rating = models.PositiveIntegerField()
    tags_ai = models.JSONField(default=list,
                               blank=True)
